  ...
  <div class="container mt-5 p-2 lg:p-5 bg-gray-300 mb-5">
    <div class="md:flex">
      <div class="w-full md:w-1/2">
        ...
        <p>{{ user.job_title }}</p>
        <NuxtLink to="profile/edit" class="bg-blue-500 hover:bg-blue-700 border-blue-500 hover:border-blue-700 text-sm border-4 text-white py-1 px-2 mt-2 rounded" type="button" @click.once="update_user">
          EDIT
        </NuxtLink>
      </div>
    ...
    <div class="grid grid-cols-4 gap-5 mt-5">
      ...
      <div class="border-l-2 border-yellow-900 pl-5">
        ...
        <ul class="mt-2">
          <li v-for="language in languages" :key="language.id"> {{ language.language_name }},</li>
        </ul>

        <h2 class="text-xl mt-5 text-blue-600 font-semibold">Hobbies</h2>
        <ul class="mt-2">
          <li v-for="hobby in hobbies" :key="hobby.id">{{ hobby.hobby_name }},</li>
        </ul>
      </div>
    </div>
  </div>
