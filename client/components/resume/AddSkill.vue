<template>
  <form class="w-full">
    <div class="flex items-center border-b border-teal-500 py-2">
      ...
      <button class="border-white bg-white border-4 text-teal-500 hover:text-teal-800 text-md py-2 px-2 rounded" type="button" @click="toggleAddNewSkill">
        Cancel
      </button>
    </div>
  </form>
...
export default Vue.extend({
  ...
  methods: {
    async add_skill ():Promise<void> {
      ...
      await this.$axios.post('skills', data).then((response) => {
        this.$toast.show({
          type: 'success',
          title: 'Success',
          message: 'Skill Added Successfully',
        })
        this.toggleAddNewSkill()
